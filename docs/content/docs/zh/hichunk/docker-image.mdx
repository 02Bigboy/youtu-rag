...
创建名为 `utu_v1.py` 的文件，包含 HiChunk 模型实现。您可以从下载的 `HiChunk` 目录或[本地部署指南](/docs/hichunk/deploying_locally)复制此文件。
...
创建名为 `registry.py` 的文件，包含更新的 vLLM 模型注册表。您可以从下载的 `HiChunk` 目录或[本地部署指南](/docs/hichunk/deploying_locally)复制此文件。
