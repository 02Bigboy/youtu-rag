...
本文档提供使用 vllm 部署 HiChunk 作为 Youtu-RAG 后端服务的说明，提供分层分块功能。
...
从[官方仓库](https://huggingface.co/tencent/HiChunk)下载预训练模型权重：
...
运行以下脚本在端口 `8501` 上使用 vLLM 服务 HiChunk 模型：
...
运行后，HiChunk 通过 OpenAI 兼容的 API 提供服务。可用的端点如下：
