...
或者，你也可以考虑在腾讯云 Ti-one 平台上部署后端服务。详情请参见[使用腾讯云 Ti-one 部署](./ti-one)。

## 步骤 1：部署 Youtu Embedding 服务
按照[本地部署 Youtu Embedding](./youtu-embedding/deploying-locally) 或[使用 Docker 部署 Youtu Embedding](./youtu-embedding/docker-image) 中的说明设置 Youtu Embedding 服务。然后将以下配置添加到你的 `.env` 文件中：
...
按照[本地部署 Youtu HiChunk](./hichunk/deploying-locally) 或[使用 Docker 部署 Youtu HiChunk](./hichunk/docker-image) 中的说明设置 Youtu HiChunk 服务。然后将以下配置添加到你的 `.env` 文件中：
...
按照[本地部署 Youtu Parsing](./youtu-parsing/deploying-locally) 或[使用 Docker 部署 Youtu Parsing](./youtu-parsing/docker-image) 中的说明设置 Youtu Parsing 服务。然后将以下配置添加到你的 `.env` 文件中：
